...
By default the database starts empty. Run `npm run seed` locally if you want to load the optional sample workspace; skip it for production.
```
...
### Deploying to Vercel (or any serverless target)

- The Express app exposes a single handler via `/api/index.js`; Vercel rewrites `/api/*` to that handler using `vercel.json`.
- On serverless runtimes we automatically store the LowDB file inside `/tmp/hti-bridge-db.json`. Override with `HTI_DB_PATH` if you prefer an external volume or managed datastore.
- Deployments boot with an empty workspace. Populate data via your ingestion jobs or run `npm run seed` manually in environments where sample content is useful.
- Add any required environment variables (e.g., API keys above) in the Vercel dashboard so the ingestion jobs and smoke tests can run remotely.
